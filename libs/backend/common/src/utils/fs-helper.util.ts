...
import { FileType } from '../typings/doc-encryption.typing';
...
export const getFileTypeFromFile = async (path: string): Promise<FileType> => await fromFile(path);
...
export const filterFilesNotInDir = async (dirPath: string, requiredRelativeFilePaths: string[]): Promise<string[]> => {
  const filesInDir = await listDirContent(dirPath);
  return requiredRelativeFilePaths.filter((file) => !filesInDir.includes(file));
};

export const filterFilesNotInDirAndSubDir = async (dirPath: string, requiredRelativeFilePaths: string[]): Promise<string[]> => {
