...
import { StyledDateInput, StyledFooter } from './style';
...
interface Props {
  ...
  setIsSecondFactorAuthModalVisible: Dispatch<SetStateAction<boolean>>;
}
...
export const FirstFactorAuthModal = forwardRef<HTMLDivElement, Props>(
  ({ onClose, onBackButtonClick, setIsSecondFactorAuthModalVisible }, ref) => {
    ...
    const {
      ...
      onSuccess: (data) => {
        const { accessToken, maskedMobile } = data;
        dispatch(setFirstFaToken(accessToken));
        dispatch(setMaskedMobile(maskedMobile));
        setIsSecondFactorAuthModalVisible(true);
      },
    });
    ...
    return (
      <Modal
        onBackdropClick={!isActivityBannedFromNonSingpassVerificationBeforeDoing1Fa ? onClose : undefined}
        trapFocus
        initialFocus={!isActivityBannedFromNonSingpassVerificationBeforeDoing1Fa ? "input[name='finNric']" : undefined}
        ref={ref}
      >
        <Modal.Card>
          <form onSubmit={handleSubmit(onSubmit)} id={FORM_IDS.NON_SINGPASS_FIRST_FA_FORM}>
            <Modal.Header onCloseButtonClick={!isActivityBannedFromNonSingpassVerificationBeforeDoing1Fa ? onClose : undefined}>
              <Modal.Title>
                {isActivityBannedFromNonSingpassVerificationBeforeDoing1Fa ? 'Access denied' : 'Verify your particulars'}
              </Modal.Title>
            </Modal.Header>
            <Modal.Body>
              {isActivityBannedFromNonSingpassVerificationBeforeDoing1Fa ? (
                ...
                <>
                  {isError && (
                    <Alert
                      variant="DANGER"
                      children={
                        isActivityBannedFromVerification(error) ? <MaxVerificationAttemptsFailedMsg /> : VERIFICATION_ATTEMPT_FAILED_TEXT
                      }
                      data-testid={
