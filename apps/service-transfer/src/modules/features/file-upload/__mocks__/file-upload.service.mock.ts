...
import { File, FileInfo, FilesUploadRequest, FileUploadAgencyInfo, OA_TYPE } from '@filesg/common';
...
// Test Service
// =============================================================================
//TODO: refactored based on the changes
export class TestFileUploadService extends FileUploadService {
  public async fileUpload(filesUploadRequest: FilesUploadRequest, jwtPayloadInRequest: FileUploadJwtPayloadInRequest) {
    return super.fileUpload(filesUploadRequest, jwtPayloadInRequest);
  }

  public async verifyAndReconcileTxnFileAndReqFile(
    reqFiles: File[],
    txnFiles: FileInfo[],
    agencyInfo: FileUploadAgencyInfo,
    s3Client: S3Client,
  ): Promise<FileUploadDetails[]> {
    return super.verifyAndReconcileTxnFileAndReqFile(reqFiles, txnFiles, agencyInfo, s3Client);
  }

  public async reconcileOaFile(reqFile: File, txnFile: FileInfo, agencyInfo: FileUploadAgencyInfo): Promise<OaFileUploadDetails> {
    return super.reconcileOaFile(reqFile, txnFile, agencyInfo);
  }

  public async reconcileUploadModeFile(reqFile: File, txnFile: FileInfo): Promise<UploadModeFileUploadDetails> {
    return super.reconcileUploadModeFile(reqFile, txnFile);
  }

  public async reconcileCopyModeFile(reqFile: File, txnFile: FileInfo, s3Client: S3Client): Promise<CopyModeFileUploadDetails> {
    return super.reconcileCopyModeFile(reqFile, txnFile, s3Client);
  }

  public async verifyS3FileExistence(files: File[], s3Client?: S3Client) {
    return super.verifyS3FileExistence(files, s3Client);
  }

  public verifyChecksum(files: FileUploadDetails[]) {
    return super.verifyChecksum(files);
  }

  public verifyMimeType(files: FileUploadDetails[]) {
    return super.verifyMimeType(files);
  }

  public verifyValidSize(files: FileUploadDetails[]) {
    return super.verifyValidSize(files);
  }

  public async transferFileToStgBucket(file: FileUploadDetails, s3Client?: S3Client) {
    return super.transferFileToStgBucket(file, s3Client);
  }

  public sqsFileAssetInfosMapper(file: FileUploadDetails) {
    return super.sqsFileAssetInfosMapper(file);
  }

  public decryptSigningKey(encryptedKey: string) {
    return super.decryptSigningKey(encryptedKey);
  }

  public appendToOaCertificateHash(oaCertificateHash: string) {
    return super.appendToOaCertificateHash(oaCertificateHash);
  }
}
...
export const oaBase64 =
  '';
export const oaChecksum = 'c6f73dbac26423aba8ab225f7b8a83d530f20aadf203bf1e3cf93233bf407ff7';
export const oaBuffer = Buffer.from([
  123, 34, 115, 105, 103, 110, 97, 116, 117, 114, 101, 34, 58, 123, 34, 116, 97, 114, 103, 101, 116, 72, 97, 115, 104, 34, 58, 34, 116, 104,
  105, 115, 45, 105, 115, 45, 111, 98, 118, 105, 111, 117, 115, 108, 121, 45, 110, 111, 116, 45, 97, 45, 104, 97, 115, 104, 34, 125, 125,
]);
export const pdfBase64 =
...
export const pdfChecksum = '3df79d34abbca99308e79cb94461c1893582604d68329a41fd4bec1885e6adb4';
export const pdfBuffer = Buffer.from(pdfBase64, 'base64');

export const oaReqFile: File = {
  fileName: 'DummyOa.oa',
  isOA: true,
  oaType: OA_TYPE.LTVP,
  fileData: oaBase64,
...
export const uploadModeReqFile: File = {
  fileName: 'DummyPdf.pdf',
  isOA: false,
  fileData: pdfBase64,
};

export const copyModeReqFile: File = {
  fileName: 'DummyCopyModeInput.pdf',
  isOA: false,
  s3FileData: {
...
export const oaTxnFile: FileInfo = {
  name: oaReqFile.fileName,
  checksum: oaChecksum,
  fileAssetId: 'fileasset-uuid-1',
  isPasswordEncryptionRequired: false,
};

export const uploadModeTxnFile: FileInfo = {
  name: uploadModeReqFile.fileName,
  checksum: pdfChecksum,
  fileAssetId: 'fileasset-uuid-2',
  isPasswordEncryptionRequired: false,
};

export const copyModeTxnFile: FileInfo = {
  name: copyModeReqFile.fileName,
  checksum: pdfChecksum,
  fileAssetId: 'fileasset-uuid-3',
  isPasswordEncryptionRequired: false,
};

export const oaFileUploadDetails: OaFileUploadDetails = {
  type: 'OA',
  name: oaReqFile.fileName,
  fileAssetId: oaTxnFile.fileAssetId,
  generatedChecksum: oaChecksum,
  expectedChecksum: oaTxnFile.checksum,
  isPasswordEncryptionRequired: oaTxnFile.isPasswordEncryptionRequired,
  size: 59,
  oaCertificateId: 'random-oa-certificate-id',
  oaCertificateHash: 'this-is-obviously-not-a-hash',
  buffer: oaBuffer,
};

export const uploadModeFileUploadDetails: UploadModeFileUploadDetails = {
  type: 'UPLOAD_MODE',
  name: uploadModeReqFile.fileName,
  fileAssetId: uploadModeTxnFile.fileAssetId,
  generatedChecksum: pdfChecksum,
  expectedChecksum: uploadModeTxnFile.checksum,
  isPasswordEncryptionRequired: uploadModeTxnFile.isPasswordEncryptionRequired,
  size: Buffer.byteLength(pdfBuffer),
  mimeType: 'application/pdf',
  buffer: pdfBuffer,
};

export const copyModeFileUploadDetails: CopyModeFileUploadDetails = {
  type: 'COPY_MODE',
  name: copyModeReqFile.fileName,
  fileAssetId: copyModeTxnFile.fileAssetId,
  generatedChecksum: pdfChecksum,
  expectedChecksum: copyModeTxnFile.checksum,
  isPasswordEncryptionRequired: copyModeTxnFile.isPasswordEncryptionRequired,
  size: 13264,
  mimeType: 'application/pdf',
  fromBucket: copyModeReqFile.s3FileData!.bucketName,
  fromKey: copyModeReqFile.s3FileData!.key,
};

export const agencyInfo: FileUploadAgencyInfo = {
  name: 'testAgency',
  code: 'TA',
  sk: 'dummy',
  identityProofLocation: 'dummy',
};

export const filesUploadRequest: FilesUploadRequest = {
  files: [oaReqFile, uploadModeReqFile, copyModeReqFile],
};

export const jwtPayloadInRequest: FileUploadJwtPayloadInRequest = {
  transactionUuid: 'random-transaction-uuid',
};

export const mockRedisFileUploadInfo = {
  userUuid: 'random-user-uuid',
  agencyInfo: agencyInfo,
  transactionInfo: {
    transactionUuid: 'random-transaction-uuid',
    files: [oaTxnFile, uploadModeTxnFile, copyModeTxnFile],
    recipients: [{ uin: 'S000000A', activityUuid: 'random-activity-uuid' }],
  },
};
export const transferFileToStgBucketError = {
  s3TransferError: {
    fileName: 'random-file-name',
    awsErrorResponse: 'random-aws-error-response',
  },
};

// type as any because the actual return is huge
export const transferFileToStgBucketSuccess: any = {
  value: {
    key: 'random-key',
    toKey: 'random-to-key',
  },
